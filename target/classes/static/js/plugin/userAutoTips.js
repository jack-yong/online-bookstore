...
var html = '<div id="autoTalkBox"'
			+' style="z-index:-2000;top:$top$px;left:$left$px;width:$width$px;height:$height$px;z-index:1;'
			+'position:absolute;scroll-top:$SCTOP$px;overflow:hidden;overflow-y:auto;visibility:hidden;'
			+'word-break:break-all;word-wrap:break-word;*letter-spacing:0.6px;">'
			+'<span id="autoTalkText"></span></div>'
			+'<div id="recipientsTips" class="recipients-tips padding5">'
			+'<div class="greyletter">从以下用户中选择或继续输入</div><ul id="autoTipsUserList"></ul></div>';
...
var AutoTips = function(A){
	...
	_this.start = function(){
  	...
  	_this.keyupFn = function(e){
		...
		var chars = val.match(/(\w+)?@([a-zA-Z0-9_\u4e00-\u9fa5]+)$|@$/);
		...
		if(char == ""){
			$.getJSON(msgAddr +"getFriendListAction?callback=?",{},function(result){
	    		...
	    		for(var i=0; i<friendList.length; i++){
	    			friendsData[i] = {user:friendList[i].userId,name:friendList[i].userName};
	    		}
	    		_this.showList(char);
			});
		}else{
	...
	_this.showList = function(char){
		key = char;
		var data = DS.inquiry(friendsData, char, friendsCount);
	...
	_this.bind = function(){
		...
		elem.onblur = function(){setTimeout(_this.hide, 1000);};
	};
	
	_this.enter = function(){
		TT.del(elem, key.length, key);
		TT.add(elem, selectList.list[selectList.index].getElementsByTagName('A')[0].title + '(' + selectList.list[selectList.index].getElementsByTagName('A')[0].rel + ')' + ' ');
		_this.hide();
		return false;
	};
	return _this;
};
window.userAutoTipsReady = function(args){
	var a = AutoTips(args);
	a.start();
	a.bind();
};
